...
def train(model,
    ...
    while iter < iters:
        for data in loader:
            ...
            experiment.log_metric('Train/lr', lr, step=iter,
                                  epoch=epoch_num)
            experiment.log_metric('Train/loss', loss.numpy()[0],
                                  step=iter, epoch=epoch_num)
            for i, value in enumerate(avg_loss_list):
                experiment.log_metric('Train/loss_'+str(i), value,
                                      step=iter, epoch=epoch_num)
            ...
            if (iter % save_interval == 0 or iter == iters) and local_rank == 0:
                ...
                experiment.log_model("PaddleSeg", current_save_dir)
                ...
                if val_dataset is not None:
                    if mean_iou > best_mean_iou:
                        ...
                        experiment.log_model("PaddleSeg", best_model_dir)
                    logger.info(
                    ...
                    if use_vdl:
                        ...
                        experiment.log_metric('val_acc', acc, step=iter,
                                              epoch=epoch_num)
                        experiment.log_metric('val_mean_iou', mean_iou,
                                              step=iter, epoch=epoch_num)
            batch_start = time.time()
